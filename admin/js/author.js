...
function displayData() {
    $.ajax({
        url: './controllers/author_json.php?data=get_author',
        type: 'GET',
        dataType: 'json',
...
$('#btnSave').click(function() {
    ...
    if(className.val() == ""){
        className.focus();
        return toastr.warning("សូមបញ្ចូលឈ្មោះអ្នកនិពន្ធ!").css("margin-top", "2rem");
    }
    // Check if txtName already exists in database
    $.ajax({
        type: 'POST',
        url: './controllers/author_json.php?data=check_author_name',
        ...
        success: function(data) {
            if (data.exists) {
                ...
                if ($('#btnSave').text() == "រក្សាទុក") {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: './controllers/author_json.php?data=add_author',
                        data: form_data,
                        dataType: 'json',
                        success: function(data) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: './controllers/author_json.php?data=update_author&id=' + class_id,
                        data: form_data,
                        dataType: 'json',
                        success: function(data) {
                            toastr.success("ជោគជ័យ").css("margin-top", "2rem");
...
function editData(id) {
    ...
    class_id = id;
    $.ajax({
        url: './controllers/author_json.php?data=get_byid',
        data: '&id=' + id,
        type: 'GET',
        dataType: 'json',
        success: function(data) {
...
function deleteData(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          type: "GET",
          url: "./controllers/author_json.php?data=delete_author&id=" + id,
          dataType: "json",
          success: function (data) {
            Swal.fire({
              title: "ជោគជ័យ",
