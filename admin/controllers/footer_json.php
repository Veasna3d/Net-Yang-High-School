<?php
    ...
    if($_GET["data"] == "get_brand"){
        ...
        while($row = $result->fetch(PDO::FETCH_ASSOC)){
          ...
          $brand[]  = array($row["id"],$row["name"],$row["image"],$row["facebook"],$row["telegram"],$row["youtube"],$row["description"],$row["phone"],$row["email"],$row["createdAt"]);
...
if($_GET["data"] == "add_brand"){
  ...
  $email = $_POST['email'];
    ...
    $sql="insert into brand(name,address,facebook,telegram,youtube,description,phone,email) 
    values(:name,:address,:facebook,:telegram,:youtube,:description,:phone,:email);";
	...
	if($_GET['data'] == 'get_byid'){
	 ...
	 if($_GET['data'] == 'update_brand'){
      ...
      $email = $_POST['email'];
			...
			$sq $sql="update brand set name=:name, address=:address, facebook=:facebook,telegram=:instagram,youtube=:youtube,description=:description,phone=:phone,email=:email where id=:id";
		
      $update->bindParam(':name',$name);
      $update->bindParam(':image', $image);
      $update->bindParam(':address',$address);
      $update->bindParam(':facebook',$fac);
      $update->bindParam(':telegram',$tele);
