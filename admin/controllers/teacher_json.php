<?php    
	...
	if ($_GET["data"] == "get_teacher") {
		...
		while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
			$user[]  = array($row["id"],$row["teacherName"],$row["password"],$row["image"],$row["gender"],$row["phone"],$row["createdAt"]);
		}
		echo json_encode($user);
	}

	if ($_GET['data'] == 'check_teacher') {
        $teacherName = $_POST['name'];
        $query = "SELECT COUNT(*) as count FROM teacher WHERE teacherName = :name";
        $statement = $conn->prepare($query);
        $statement->bindValue(':name', $teacherName);
        $statement->execute();
...
// 				$user[]  = array($row["id"],$row["username"],$row["password"],$row['role'],$row["email"],$row["image"],$row["createdAt"]);
// 			}
// 			echo json_encode($user);
// 		}
...
// 			$sql = "UPDATE user SET username=:username, password=:password,image=:image,role=:role,email=:email  where id=:id;";
